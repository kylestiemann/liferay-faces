...
/**
 * <p>
 * This abstract class implements the {@link com.liferay.faces.bridge.filter.liferay.LiferayURLGenerator} contract for
 * generating URLs that are compatible with Liferay Portal. The main reason why this is necessary is because the Liferay
 * Portal's com.liferay.portlet.PortletURLImpl#toString() method returns different values depending on the portlet
 * lifecycle phase during which it is called. Additionally, it sometimes includes the public render parameters in the
 * URL (which are not required for JSF portlets). Another issue is related to ICEfaces, which uses a server-side
 * DOM-diff algorithm within it's {@link RenderKit}. When DOM-diff strategies are used, it is critical that URLs always
 * be the same regardless of which portlet lifecycle phase is executing. For example, a URL generated during the
 * {@link PortletRequest#RENDER_PHASE} of the portlet lifecycle must be identical to the one generated during the
 * {@link PortletRequest#RESOURCE_PHASE}, or else a DOM-diff will be detected.
 * </p>
 * <p>
 * The constructors of this class receive a String-based parameter named "baseURL" which is expected to be the result of
 * calling the toString() method of a Liferay URL. The baseURL is then parsed and the URL parameters are cached so that
 * the {@link #generateURL(Map)} method can quickly generate a consistent URL.
 * </p>
 *
 * @author Neil Griffin
 */
public abstract class LiferayURLGeneratorBaseImpl implements LiferayURLGenerator {
	 ...
	 * Constructs a new instance.
	 *
	 * @param baseURL The String-based URL generated by Liferay Portal's com.liferay.portlet.PortletURLImpl#toString()
	 *            method. method.
	...
	public String generateURL(Map<String, String[]> additionalParameterMap, String cacheability,
		...
		if (baseURL.contains(Portal.FRIENDLY_URL_SEPARATOR)) {
			...
			String portalAuthToken =
				StringHelper.toString(additionalParameterMap.get(P_AUTH), parameterMap.get(P_AUTH));
			...
			String portletAuthToken =
				StringHelper.toString(additionalParameterMap.get(P_P_AUTH), parameterMap.get(P_P_AUTH));
			...
			String stateRestoreCurrentView =
				StringHelper.toString(additionalParameterMap.get(P_P_STATE_RCV), parameterMap.get(P_P_STATE_RCV));
			...
			if (LIFECYCLE_RESOURCE_PHASE_ID.equals(portletLifecycleId)) {
				...
				if (urlCacheability == null) {
					urlCacheability =
						StringHelper.toString(additionalParameterMap.get(P_P_CACHEABILITY),
			...
			String outerPortletId =
				StringHelper.toString(additionalParameterMap.get(P_O_P_ID), parameterMap.get(P_O_P_ID));
			...
			String doAsUserId =
				StringHelper.toString(additionalParameterMap.get(DO_AS_USER_ID), parameterMap.get(DO_AS_USER_ID));
			...
			String doAsUserLanguageId =
				StringHelper.toString(additionalParameterMap.get(DO_AS_USER_LANGUAGE_ID),
			...
			String doAsGroupId =
				StringHelper.toString(additionalParameterMap.get(DO_AS_GROUP_ID), parameterMap.get(DO_AS_GROUP_ID));
			...
			String refererGroupId =
				StringHelper.toString(additionalParameterMap.get(REFERER_GROUP_ID), parameterMap.get(REFERER_GROUP_ID));
			...
			String refererPlid =
				StringHelper.toString(additionalParameterMap.get(REFERER_PLID), parameterMap.get(REFERER_PLID));
			...
			String controlPanelCategory =
				StringHelper.toString(additionalParameterMap.get(CONTROL_PANEL_CATEGORY),
