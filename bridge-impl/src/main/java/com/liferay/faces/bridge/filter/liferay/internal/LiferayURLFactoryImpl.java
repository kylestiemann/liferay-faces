 ...
 * ensuring that the toString() method of {@link MimeResponse#createActionURL()}, {@link MimeResponse#createRenderURL()}
 * , and {@link MimeResponse#createResourceURL()} are called only once during the JSF lifecycle, and that the pertinent
 * parts of the String are cached. However, the optimization is only usable for portlets that do not have an associated
 * Liferay {@link FriendlyURLMapper}. For more info, see FACES-257.
 *
 * @author Neil Griffin
 */
public class LiferayURLFactoryImpl extends LiferayURLFactory {
	...
	public LiferayActionURL getLiferayActionURL(BridgeContext bridgeContext, MimeResponse mimeResponse,
		...
		LiferayURLGenerator liferayURLGenerator =
			(LiferayURLGenerator) bridgeContext.getAttributes().get(ACTION_URL_GENERATOR);

		if (liferayURLGenerator == null) {

			PortletURL actionURL = mimeResponse.createActionURL();
			liferayURLGenerator =
				new LiferayURLGeneratorActionImpl(actionURL.toString(), actionURL.getPortletMode(), responseNamespace,
	...
	public LiferayRenderURL getLiferayRenderURL(BridgeContext bridgeContext, MimeResponse mimeResponse,
		...
		if (friendlyURLMapperEnabled) {
			...
			LiferayURLGenerator liferayURLGenerator =
				(LiferayURLGenerator) bridgeContext.getAttributes().get(RENDER_URL_GENERATOR);

			if (liferayURLGenerator == null) {

				PortletURL renderURL = mimeResponse.createRenderURL();
				liferayURLGenerator =
					new LiferayURLGeneratorRenderImpl(renderURL.toString(), renderURL.getPortletMode(),
						responseNamespace, renderURL.getWindowState());
	...
	public LiferayResourceURL getLiferayResourceURL(BridgeContext bridgeContext, MimeResponse mimeResponse,
		...
		LiferayURLGenerator liferayURLGenerator =
			(LiferayURLGenerator) bridgeContext.getAttributes().get(RESOURCE_URL_GENERATOR);
