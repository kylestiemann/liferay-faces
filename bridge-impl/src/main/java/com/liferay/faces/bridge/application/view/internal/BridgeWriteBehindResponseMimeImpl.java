 ...
 * feature is responsible for rendering of markup that appears after the closing </f:view> component tag.
 * </p>
 * <p>
 * The Portlet Bridge Spec requires that the {@link Bridge#AFTER_VIEW_CONTENT} feature be supported by implementing the
 * {@link BridgeWriteBehindResponse} interface. However with it is possible to have the JSF implementation (Mojarra or
 * MyFaces) handle the entire interleaving process by itself. So although this class implements the
 * {@link BridgeWriteBehindResponse} interface, it only does so for the sake of completeness. The corresponding methods
 * implemented in this class throw {@link UnsupportedOperationException} which is OK since they will never be called.
 * </p>
 * <p>
 * In order to have the JSF implementation handle the entire interleaving process by itself, it is necessary to overcome
 * some Servlet API dependencies. As part of this mechanism, the {@link #getPortletOutputStream()} and
 * {@link #getWriter()} methods in this class delegate writing of the response to the underlying {@link ServletResponse}
 * .
 * </p>
 *
 * @author Neil Griffin
