...
/**
 * <p>
 * This class is a JSF {@link PhaseListener} that listens to the {@link PhaseId#INVOKE_APPLICATION} and
 * {@link PhaseId#RENDER_RESPONSE} phases of the JSF lifecycle. Along with {@link HeadManagedBean} and
 * {@link HeadRendererBridgeImpl}, this class helps provides a solution to an issue regarding Ajax-initiated execution
 * of navigation-rules in a portlet. When a portal page is first rendered by the portal, all of the portlets on the page
...
public class HeadPhaseListener implements PhaseListener {
	...
	/**
	 * <p>
	 * This method is called before the {@link PhaseId#RENDER_RESPONSE} phase of the JSF lifecycle is executed. The
	 ...
	 * {@link PhaseId#APPLY_REQUEST_VALUES} phase (in the case of immediate="true") or the
	 * {@link PhaseId#INVOKE_APPLICATION} phase (in the case of immediate="false") has completed. If this is the case,
	 * then the list of head resourceIds in the {@link HeadManagedBean} needs to be repopulated from the list found in
	 * the Flash scope.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * This method is called before the {@link PhaseId#APPLY_REQUEST_VALUES} phase of the JSF lifecycle is executed. The
	 * purpose of this timing is to handle the case when the user clicks on a {@link UICommand} component (like
	 ...
	 * </p>
	 * <p>
	 * <code>&lt;f:ajax execute="@form" render=" @form" /&gt;</code>
	 * </p>
	 * <p>
	 * When this happens, we need to somehow remember the list of JavaScript and/or CSS resources that are currently in
	 * the &lt;head&gt; section of the portal page. This is because a navigation-rule might fire which could cause a new
	 * view to be rendered in the {@link PhaseId#RENDER_RESPONSE} phase that is about to follow this
	 * {@link PhaseId#APPLY_REQUEST_VALUES} phase. The list of resources would be contained in the
	 * {@link HeadManagedBean} {@link ViewScoped} instance that is managed by the JSF managed-bean facility. The list
	 * would have been populated initially in the {@link HeadManagedBean} by the {@link HeadRender} during the initial
	 * HTTP-GET of the portal page. The way we "remember" the list is by placing it into the JSF 2 {@link Flash} scope.
	 * This scope is used because it is very short-lived and survives any navigation-rules that might fire, thereby
	 * causing the rendering of a new JSF view.
	 * </p>
	 * <p>
	 * The story is continued in the {@link #beforeRenderResponsePhase(PhaseEvent)} method below...
	 * </p>
	 */
