...
//		FolderTreeRootNode folderTreeRootNode = docLibModelBean.getLazyNodeDataModel().getFolderTreeRootNode();
//
//		//TODO magic number?
//		if (parentFolder.getFolderId() == 0) {
//			System.err.println("QWERQWERQWERQWERQWERQWERQWER");
//			parentFolderTreeNode = folderTreeRootNode;
//		}
//
//		// TODO bug
//
//		// docLibModelBean.getLazyNodeDataModel().mySetData(parentFolderTreeNode);
//		// docLibModelBean.getLazyNodeDataModel().navToChild(parentFolderTreeNode.getParent().getIndex(parentFolderTreeNode));
...
//
//		// docLibViewBean.getTreeNodeStateMap().put(childFolderTreeNode, new
//		// NodeState());
...
//		
//
//		// docLibViewBean.setRootFolderSelected(docLibModelBean.getSelectedFolder().equals((Folder)folderTreeRootNode.getUserObject()));
...
//		docLibViewBean.getTreeNodeStateMap().get(parentFolderTreeNode).setExpanded(true);
	...
	public void delete(ActionEvent actionEvent) {
		...
		if (docLibViewBean.isFilesSelected()) {
			docLibViewBean.setDeleteFileDialogRenderedTrue();
		}
		else {
			docLibViewBean.setDeleteFolderDialogRenderedTrue();
		}
	}
	...
	public void deleteFiles() {
		...
		docLibViewBean.setDeleteFileDialogRenderedFalse();
	}
	...
	public void handleFileUpload(FileEntryEvent fileEntryEvent) {
		...
		if (!cancel) {
			...
			try {
				org.icefaces.ace.component.fileentry.FileEntry fileEntry =
					(org.icefaces.ace.component.fileentry.FileEntry) fileEntryEvent.getSource();
				...
				for (FileEntryResults.FileInfo fileInfo : results.getFiles()) {
					...
					if (uploadedFile.getStatus() == UploadedFile.Status.FILE_SAVED) {
						...
						try {
							...
							serviceContext.setAttribute("fileEntryTypeId", docLibModelBean.getFileEntryTypeId());

							// TODO permssions?
							// Temporary: Make the default setting be that
							// community
							// members can view the file. Need to
							// develop a "Viewable By" permissions Facelet
							// composite
	...
	public void selectFolder(Folder folder) {
		...
		try {
			...
			// Folder rootFolder =
			// (Folder)docLibModelBean.getLazyNodeDataModel().getFolderTreeRootNode().getUserObject();
			// TODO magic number
			docLibViewBean.setRootFolderSelected(docLibModelBean.getSelectedFolder().getFolderId() == 0);
